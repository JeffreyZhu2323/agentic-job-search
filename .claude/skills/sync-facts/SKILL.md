...
4. **Write on approval.** Put only the lines Jeffrey approves into `resume/Resume-Facts.md` (CAN CLAIM section), reworded to match the file's existing style, following its **Write-back policy** (top of that file: reusable across applications, not already captured; prefer updating an existing fact over adding a near-duplicate). Nothing unreviewed goes in.
...
6. **Record what you reconciled** so the hook stops flagging it. Use the same computation the hook uses (so values match by construction):
   - **`gdoc` source:** `pdftotext "<localfile>" - | sha256sum | cut -d' ' -f1 > assets/.src-<name>.hash` — hash the extracted **text**, never the raw PDF: Google Docs re-stamps fresh metadata on every export, so `sha256sum` of the raw PDF changes on every download and would flag the source "changed" forever. The hook hashes the text the same way, so a content-based baseline matches by construction.
